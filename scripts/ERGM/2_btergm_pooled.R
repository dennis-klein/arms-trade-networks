#------------------------------------------------------------------------------#
# Model estimation BTERGM Pooled over the complete period (not pursued further)
#------------------------------------------------------------------------------#
...
# setup standard errors (set higher if later used)
n_boot <- 100
n_sim <- 100
...
# we only include countries present over the complete observation period
...
# helper: matrices for kronecker products
...
# init empty covariates, list over years
netC <- list()
netD <- list()
...
for (year in (start + maxlag):end){
  ...
  # dependent multilayer network, one for import one for export
  # threshold for dependency is set at 1pct.
  netC[[j]] = to.multiplex(
...
# output with kableExtra -> construct dataframe with results
...
# output latex table
sink(file = "figures/tergm_pooled_2.txt")
...
pdf(file = "figures/tergm_gof_2.pdf", width = 10, height = 0.9 * sqrt(2) * 10)
par(mfrow = c(3,2))
plot(gof_full)
dev.off()