...
itpd_mining = readRDS("data/out/itpd_mining-energy.rds")
...
for (i in c(1, 6, 11)) {
  plot(networks_yearly_itpd[[i]],
       ...
       main = paste("International Trade Energy / Miningin", i + 1994))
}
dev.off()
...
plot(1995:2018, unlist(lapply(networks_yearly_sipri, FUN = function(x){sum(get.edge.value(x,"TIV (in thousands)"))})),  type = "l", xlab = "Time", ylab = "Series 1", main="Sum TIV (in thousands)")
plot(1995:2018, unlist(lapply(networks_yearly_cepii, FUN = function(x){sum(get.edge.value(x,"Trade Flow"))})),  type = "l", xlab = "Time", ylab = "Series 1", main="Sum of Conventional Trade Flow")

plot(2000:2016, unlist(lapply(networks_yearly_itpd, FUN = function(x){network.edgecount(x)})), type = "l", xlab = "Time", ylab = "Series 1", main = "ITPD Mining/Energy: Edge Count")
plot(2000:2016, unlist(lapply(networks_yearly_itpd, FUN = function(x){network.density(x)})), type = "l", xlab = "Time", ylab = "Series 1", main = "ITPD Mining/Energy: Network Density")
plot(2000:2016, unlist(lapply(networks_yearly_itpd, FUN = function(x){sum(get.edge.value(x,"ITPD Mining/Energy"))})),  type = "l", xlab = "Time", ylab = "Series 1", main="ITPD Mining/Energy Trade Value Sum")
